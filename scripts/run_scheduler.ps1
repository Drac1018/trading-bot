...
function Test-ExplicitDatabaseConfiguration {
    ...
    if ([string]::IsNullOrWhiteSpace($databaseUrl)) {
        throw "DATABASE_URL is required for scheduler startup. This script no longer falls back to SQLite. Set a PostgreSQL URL, or set DATABASE_URL to sqlite://... together with TRADING_MVP_ALLOW_SQLITE=1 for explicit local/dev use."
    }
    ...
    if ($databaseUrl -like "sqlite*") {
        ...
        if ($allowSqlite -notin @("1", "true", "TRUE", "True", "yes", "YES", "Yes", "on", "ON", "On")) {
            throw "DATABASE_URL points to SQLite. Scheduler startup requires explicit opt-in for SQLite. Re-run with TRADING_MVP_ALLOW_SQLITE=1 only for local/dev use."
        }
...
Invoke-CheckedPython -Arguments @("workers\scheduler.py")
