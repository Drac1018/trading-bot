...
### Non-live execution surfaces (hard-disabled)

아래 API는 현재 제품 범위(실거래 코어 운영) 밖으로 분류되어 모두 `410 Gone`으로 하드 비활성화됩니다.
...
- `detail.code`: `NON_LIVE_SURFACE_DISABLED`
- `detail.message`: 비활성화된 endpoint와 사유(현재 live trading 제품 범위 밖)를 포함
...
### Replay validation data sources (offline analysis only)

`/api/replay/validation`는 하드 비활성화되었고, 아래 내용은 오프라인 분석 코드 경로(예: service / CLI)에만 해당합니다.
...
`python -m trading_mvp.cli replay-compare` returns:
