...
제품 개선·기획 성격의 보조 흐름은 현재 활성 제품 범위에 포함하지 않는다.
...
6. 시장/계좌/포지션 수집, AI 판단, `risk_guard`, 실행 통제, 감사 로그, 수동 제어를 현재 제품 범위로 유지한다.
7. 기본 신규 진입 bias는 `scalp/intraday`로 유지하고, `swing/position`은 강한 구조 정렬이 있을 때만 예외적으로 허용한다.
8. 최초 손절은 항상 deterministic hard stop + 거래소 resident protective stop 기준으로 유지하고, AI는 tighten/reduce 제안만 할 수 있다.
...
- averaging down은 금지하고, add-on은 winner-only pyramiding일 때만 허용한다.
- stop widening, hard stop 제거, protection 없는 포지션 허용은 금지한다.
...
- 새 운영 문서를 추가하거나 크게 수정할 때 UI 노출 내부 키가 직접 등장하면, 문서 상단에 `운영자 표현과 내부 키` 섹션을 두고 형식은 `docs/operator-bridge-template.md`를 재사용한다.
