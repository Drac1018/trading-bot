...
function BreakdownTable({
  ...
  return (
    <div className="rounded-[1.4rem] border border-amber-200 bg-white px-4 py-4">
      ...
      {roles.length === 0 ? (
        <p className="mt-3 text-sm text-slate-500">아직 관측된 AI 호출이 없습니다.</p>
      ) : (
    ...
    <section className="space-y-5 rounded-[1.9rem] border border-amber-200/70 bg-canvas/70 p-4 sm:p-5">
      <div className="flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600">
            설정 기반 월간 추정치와 최근 24시간/7일 관측값을 같이 보여줍니다. 수동 실행은 최소{" "}
            {settings.manual_ai_guard_minutes}분 간격으로 보호됩니다.
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard
          ...
          hint="AI를 켠 경우 역할별 추정치를 합산한 값"
        />
        <MetricCard
          label="수동 실행 가드"
          value={`${formatNumber(settings.manual_ai_guard_minutes)}분`}
          hint="반복 클릭으로 인한 과호출을 막는 최소 간격"
        />
      </div>
          ...
          <div className="mt-3 space-y-3">
            {Array.from(
              ...
              >
                ...
                <p>실측 환산 {formatNumber(settings.observed_monthly_ai_calls_projection_breakdown[role] ?? 0)}회</p>
              </div>
            ))}
          </div>
        </div>
