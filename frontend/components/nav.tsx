...
const items = [
  ...
  { href: "/dashboard/scheduler", label: "스케쥴러 상태" },
  { href: "/dashboard/agents", label: "에이전트 디버그" },
    ...
    <nav className="rounded-[2rem] border border-amber-300/60 bg-panel/95 p-3 shadow-frame backdrop-blur">
      <div className="rounded-[1.6rem] bg-ink px-4 py-5 text-canvas sm:px-5 sm:py-6">
        ...
        <p className="mt-3 text-sm leading-6 text-canvas/80">
          실거래 상태, 리스크 차단, 승인, 스케줄, 감사 로그를 운영 흐름 기준으로 분리해서 확인합니다.
        </p>
      </div>
            ...
            {items.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`whitespace-nowrap rounded-full border px-4 py-2 text-sm font-medium transition ${itemClass(active)}`}
                >
                  {item.label}
                </Link>
              );
        ...
        {items.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`rounded-2xl border px-4 py-3 text-sm font-medium transition ${itemClass(active)}`}
            >
              {item.label}
            </Link>
          );
