...
test("cost breakdown page renders monthly summary and daily buckets", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "기간별 수익성 비용 분해" })).toBeVisible();
  await expect(page.getByRole("heading", { name: "월간 수익성 비용 분해" })).toBeVisible();
  await expect(page.getByRole("cell", { name: "2026-05-01" })).toBeVisible();
  await expect(page.getByText("오늘 카드로 돌아가기")).toBeVisible();
});
...
test("cost breakdown page renders yearly summary and monthly buckets", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "기간별 수익성 비용 분해" })).toBeVisible();
  await expect(page.getByText("연간 수익성 비용 분해")).toBeVisible();
  await expect(page.getByRole("cell", { name: "2026-05" })).toBeVisible();
  await expect(page.getByText("Fee / Gross")).toBeVisible();
});
