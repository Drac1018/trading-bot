...
test("buildOperatorDetailSections exposes the five operator sections in stable order", async () => {
  ...
  assert.deepEqual(
    ...
    ["현재 레짐", "파생 / 오더북 요약", "이벤트 리스크", "리스크 가드 판정", "차단 / 저하 사유"],
  );
});
...
test("buildOperatorDetailSections shows source unavailable explicitly in event risk context", async () => {
  ...
  assert.equal(
    eventSection.items.find((item) => item.label === "소스 상태")?.value,
    "소스 없음",
  );
